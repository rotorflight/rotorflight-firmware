...
static void governorUpdatePassthrough(void)
{
    ...
    if (!ARMING_FLAG(ARMED)) {
        ...
        switch (gov.state)
        {
            ...
            case GS_THROTTLE_IDLE:
                govMain = slewLimit(govPrev, gov.throttleInput, gov.throttleTrackingRate);
                if (gov.throttleInputLow)
                    govChangeState(GS_THROTTLE_OFF);
...
static void governorUpdateState(void)
{
    ...
    if (!ARMING_FLAG(ARMED) || getBatteryCellCount() == 0) {
        ...
        switch (gov.state)
        {
            ...
            //  -- if throttle > 20%, move to SPOOLUP
            case GS_THROTTLE_IDLE:
                govMain = slewUpLimit(govPrev, idleMap(gov.throttleInput), gov.throttleSpoolupRate);
...
void governorInitProfile(const pidProfile_t *pidProfile)
{
    if (getMotorCount() > 0)
    {
        ...
        gov.TTAGain   = pidProfile->governor.tta_gain / -250.0f;
...
void governorInit(const pidProfile_t *pidProfile)
{
    ...
    if (getMotorCount() > 0)
    {
        ...
        gov.zeroThrottleTimeout  = governorConfig()->gov_lost_throttle_timeout * 100;
