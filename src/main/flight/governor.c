...
void governorInit(void)
{
    ...
    if (getMotorCount() > 0)
    {
        ...
        biquadFilterInitLPF(&govVoltageFilter, governorConfig()->gov_pwr_filter, targetPidLooptime);
        biquadFilterInitLPF(&govCurrentFilter, governorConfig()->gov_pwr_filter, targetPidLooptime);
        biquadFilterInitLPF(&govMotorRPMFilter, governorConfig()->gov_rpm_filter, targetPidLooptime);
    }
}
