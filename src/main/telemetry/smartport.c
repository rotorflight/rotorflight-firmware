...
#define SMARTPORT_SERVICE_TIMEOUT_US    1000
#define SMARTPORT_MSP_SILENCE_US        250000
...
{
    uint32_t lat = abs(gpsSol.llh.lat) * 6 / 100;
...
{
    uint32_t lon = abs(gpsSol.llh.lon) * 6 / 100;
...
static telemetrySensor_t smartportTelemetrySensors[] =
{
    ...
    TLM_SENSOR(ESC1_CURRENT,            0x0208,   100,  3000,   1,  10,   100,  INT),
    TLM_SENSOR(ESC1_CAPACITY,           0x5258,   100,  3000,   1,  10,   0,    INT),
    TLM_SENSOR(ESC1_ERPM,               0x0508,   100,  3000,   1,  10,   0,    INT),
    TLM_SENSOR(ESC1_POWER,              0x5268,   100,  3000,   1,  10,   0,    INT),
    ...
    TLM_SENSOR(BUS_VOLTAGE,             0x0902,   100,  3000,   1,  10,   10,   INT),

    TLM_SENSOR(ESC_VOLTAGE,             0x0211,   100,  3000,   1,  10,   10,   INT),
    TLM_SENSOR(ESC_CURRENT,             0x0201,   100,  3000,   1,  10,   100,  INT),
    TLM_SENSOR(BEC_CURRENT,             0x0222,   100,  3000,   1,  10,   0,    INT),
    ...
    TLM_SENSOR(RT_LOAD,                 0x51D2,   200,  3000,   1,  10,   10,   INT),
    ...
    TLM_SENSOR(BATTERY_TYPE,            0x5133,   200,  3000,   1,   1,   0,    INT),
    ...
    TLM_SENSOR(DEBUG_3,                 0x52F3,   100,  3000,   1,  10,   0,    INT),
...
void processSmartPortTelemetry(smartPortPayload_t *payload, volatile bool *clearToSend, const timeUs_t *requestTimeout)
{
    ...
    if (*clearToSend) {
        ...
        if (smartPortMspReplyPending) {
            ...
            // Don't send sensor data for a while
            nextSensorSendTime = micros() + SMARTPORT_MSP_SILENCE_US;
            *clearToSend = false;
            return;
        }
...
void handleSmartPortTelemetry(timeUs_t currentTimeUs)
{
    if (telemetryState != TELEMETRY_STATE_UNINITIALIZED) {
        ...
        if (telemetryState == TELEMETRY_STATE_INITIALIZED_SERIAL && smartPortSerialPort) {
            ...
            if (clearToSend) {
                const smartPortPayload_t emptySmartPortFrame = INIT_ZERO;
