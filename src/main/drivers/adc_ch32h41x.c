...
static volatile DMA_DATA uint32_t adcConversionBuffer[ADC_CHANNEL_COUNT]; // one entry per AdcChannel
...
{
    for (unsigned i = 0; i < ADC_CHANNEL_INTERNAL_FIRST_ID; i++)
    {
 ...
 */
uint16_t adcInternalRead(int source)
{
    switch (source)
    {
    case ADC_VREFINT:
