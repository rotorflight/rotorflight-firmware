 ...
 *
 * Based on betaflight CH32H41x WS2811 LED strip driver by Temperslee.
 * Adapted for rotorflight API (ioTag_t parameter).
 */
...
#if defined(USE_LED_STRIP) && (defined(CH32H4) || defined(CH32H41x))
...
#if defined(USE_DMA_SPEC)
    dmaMuxEnable(dmaGetIdentifier(dmaRef), dmaSpec->channel);
#endif
...
#endif // USE_LED_STRIP && (CH32H4 || CH32H41x)
