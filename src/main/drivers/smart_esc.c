...
void smartescSetThrottleRateHz(uint32_t rateHz)
{
    ...
    smartescConfigMutable()->throttle_rate_hz = (uint16_t)(1000000 / smartescThrottleIntervalUs);
}
...
bool smartescInit(const smartesc_rxConfig_t *rxConfig, smartesc_rxRuntimeState_t *smartescRs)
{
    ...
    // Apply persisted SMART ESC rates on startup
...
/* Called from esc_sensor.c processing loop to allow SMART ESC code to parse queued frames */
...
/* Called periodically (service) from esc_sensor.c to perform transmissions or housekeeping */
