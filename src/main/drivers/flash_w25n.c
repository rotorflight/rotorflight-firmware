...
#define W25N_MAX_SPI_CLK_HZ             104000000
...
static void w25n_performCommandWithPageAddress(flashDeviceIO_t *io, uint8_t command, uint32_t pageAddress)
{
    if (io->mode == FLASHIO_SPI) {
        ...
        quadSpiInstructionWithAddress1LINE(quadSpi, command, 0, pageAddress, W25N_STATUS_PAGE_ADDRESS_SIZE + 8);
    }
#endif
}
...
bool w25n_detect(flashDevice_t *fdevice, uint32_t jedecID)
{
    ...
    for (index = 0; w25nFlashConfig[index].jedecID; index++) {
        if (w25nFlashConfig[index].jedecID == jedecID) {
            geometry->sectors = w25nFlashConfig[index].blocks;
            geometry->pagesPerSector = W25N_PAGES_PER_BLOCK;
    ...
    if (w25nFlashConfig[index].reserved) {
        flashPartitionSet(FLASH_PARTITION_TYPE_BADBLOCK_MANAGEMENT,
                fdevice->geometry.sectors - w25nFlashConfig[index].reserved,
...
uint32_t w25n_pageProgramContinue(flashDevice_t *fdevice, uint8_t const **buffers, uint32_t *bufferSizes, uint32_t bufferCount)
{
    if (bufferCount < 1) {
        fdevice->callback(0);
        return 0;
...
static void w25n_deviceInit(flashDevice_t *flashdev)
{
    ...
    spiSetClkDivisor(flashdev->io.handle.dev, spiCalculateDivider(W25N_MAX_SPI_CLK_HZ));
}
