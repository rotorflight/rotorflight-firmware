        ...
        if (IS_RC_MODE_ACTIVE(i)) {
            ...
            case BOXCAMERA1:
                if (isFeatureSupported(RCDEVICE_PROTOCOL_FEATURE_SIMULATE_WIFI_BUTTON)) {
                    ...
                    if (!ARMING_FLAG(ARMED) && !(getArmingDisableFlags() & ARMING_DISABLED_CRASH_DETECTED)) {
...
static void rcdevice5KeySimulationProcess(timeUs_t currentTimeUs)
{
    ...
    if (ARMING_FLAG(ARMED) || IS_RC_MODE_ACTIVE(BOXSTICKCOMMANDDISABLE) || (getArmingDisableFlags() & ARMING_DISABLED_CRASH_DETECTED)) {
        return;
    }
