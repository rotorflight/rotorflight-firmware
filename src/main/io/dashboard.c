...
static void drawRxChannel(uint8_t channelIndex, uint8_t width)
{
    ...
    const int16_t value = constrain(rcInput[channelIndex], RX_PWM_PULSE_MIN, RX_PWM_PULSE_MAX);
    const uint32_t percentage = 100 * (value - RX_PWM_PULSE_MIN) / (RX_PWM_PULSE_MAX - RX_PWM_PULSE_MIN);
...
{
    for (int channelIndex = 0; channelIndex < activeRcChannelCount && channelIndex < RX_CHANNELS_PER_PAGE_COUNT; channelIndex += 2) {
        i2c_OLED_set_line(dev, (channelIndex / 2) + PAGE_TITLE_LINE_COUNT);

        drawRxChannel(channelIndex, HALF_SCREEN_CHARACTER_COLUMN_COUNT);

        if (channelIndex >= activeRcChannelCount) {
            continue;
        }
...
static void showBatteryPage(void)
{
    ...
    if (batteryConfig()->voltageMeterSource != VOLTAGE_METER_NONE) {
        ...
        uint8_t batteryPercentage = getBatteryChargeLevel();
        i2c_OLED_set_line(dev, rowIndex++);
    ...
    if (batteryConfig()->currentMeterSource != CURRENT_METER_NONE) {
        ...
        uint8_t capacityPercentage = getBatteryChargeLevel();
        i2c_OLED_set_line(dev, rowIndex++);
