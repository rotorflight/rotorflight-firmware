...
#define PROBLEM_MOTOR_PROTOCOL_DISABLED 1
    ...
    case MSP_ESC_SENSOR_CONFIG:
        ...
        sbufWriteU16(dst, escSensorConfig()->currentCorrectionFactor);
        sbufWriteU16(dst, escSensorConfig()->consumptionCorrectionFactor);
        break;
...
static mspResult_e mspProcessInCommand(mspDescriptor_t srcDesc, int16_t cmdMSP, sbuf_t *src)
{
    ...
    case MSP_SET_ESC_SENSOR_CONFIG:
        ...
        if (sbufBytesRemaining(src) >= 4) {
            escSensorConfigMutable()->currentCorrectionFactor = sbufReadU16(src);
            escSensorConfigMutable()->consumptionCorrectionFactor = sbufReadU16(src);
        }
        break;
