...
{
    if (escSensorConfig()->currentCorrectionFactor == 100)
        return current;

    return (uint32_t)lrintf((escSensorConfig()->currentCorrectionFactor / 100.0f) * current);
}

static uint32_t applyConsumptionCorrectionFactor(uint32_t consumption)
{
    if (escSensorConfig()->consumptionCorrectionFactor == 100)
        return consumption;

    return (uint32_t)lrintf((escSensorConfig()->consumptionCorrectionFactor / 100.0f) * consumption);
}
