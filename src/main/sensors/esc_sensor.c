...
static bool blheliSParamCached[MAX_SUPPORTED_MOTORS] = {false};
static bool blheliSParamWritten[MAX_SUPPORTED_MOTORS] = {false};
static uint8_t blheliSParamBuffers[MAX_SUPPORTED_MOTORS][BLHELI_S_NUM_EEPROM_BYTES];
...
static bool blheliSWritePending = false;
static bool blheliSWriteTaskEnabledByUs = false;
static uint8_t blheliSWriteEscId = MAX_SUPPORTED_MOTORS;
...
static bool am32IfFetchData(uint8_t escID)
{
    bool retVal = false;
...
static bool am32IfWriteData(uint8_t escID)
{
    bool retVal = false;
...
static bool blheliSIfFetchData(uint8_t escID)
{
    ...
    if (blheliSParamCached[escID]) {
        memcpy(paramPayload, blheliSParamBuffers[escID], BLHELI_S_NUM_EEPROM_BYTES);
            ...
            if (devInfo != NULL) {
                ...
                if (matchedEepromAddr != 0) {
                    ...
                    if (fwifCmdDeviceRead(BLHELI_S_NUM_EEPROM_BYTES, paramPayload, matchedEepromAddr)) {
                        retVal = true;
                        escSig = ESC_SIG_BLHELI_S;
                        memcpy(blheliSParamBuffers[escID], paramPayload, BLHELI_S_NUM_EEPROM_BYTES);
                        blheliSParamCached[escID] = true;
                        blheliSParamWritten[escID] = false;
                        paramPayloadLength = BLHELI_S_NUM_EEPROM_BYTES;
                        paramBufferEscID = escID;
                    }
                }
...
static bool blheliSIfWriteData(uint8_t escID)
{
    bool retVal = false;
    ...
    if (pwmMotors[escID].enabled && escID < fourwayEscCount) {
        if (pwmMotors[escID].io != IO_NONE) {
            ...
            if (devInfo != NULL) {
                const uint32_t matchedEepromAddr = fwifGetBlheliSEepromAddress(devInfo);
                const uint8_t matchedPageErase = fwifGetBlheliSPageEraseIndex(devInfo);
                    ...
                    do {
                        ...
                        if (fwifCmdDeviceWrite(BLHELI_S_NUM_EEPROM_BYTES, paramUpdPayload, matchedEepromAddr)) {
                            ...
                            if (fwifCmdDeviceRead(BLHELI_S_NUM_EEPROM_BYTES, verifyBuffer, matchedEepromAddr) &&
                                ...
                                blheliSParamWritten[escID] = true;
                                break;
                            }
                        }
...
static void scheduleBlheliSWrite(uint8_t id)
{
    ...
    getTaskInfo(TASK_ESC_SENSOR, &escTaskInfo);
    if (!escTaskInfo.isEnabled) {
        blheliSWriteTaskEnabledByUs = true;
        rescheduleTask(TASK_ESC_SENSOR, TASK_PERIOD_MS(1));
        setTaskEnabled(TASK_ESC_SENSOR, true);
    } else {
        blheliSWriteTaskEnabledByUs = false;
    }
}

static bool fourwayIfFetchData(uint8_t escID)
{
    if (am32IfFetchData(escID)) {
        return true;
    }

    return blheliSIfFetchData(escID);
}
...
void escSensorProcess(timeUs_t currentTimeUs)
{
    if (am32WritePending) {
        ...
        am32IfWriteData(am32WriteEscId);
        if (am32WriteTaskEnabledByUs) {
            am32WriteTaskEnabledByUs = false;
            if (!featureIsEnabled(FEATURE_ESC_SENSOR) && escSensorPort == NULL && !blheliSWritePending) {
    ...
    if (blheliSWritePending) {
        blheliSWritePending = false;
        blheliSIfWriteData(blheliSWriteEscId);
        if (blheliSWriteTaskEnabledByUs) {
            blheliSWriteTaskEnabledByUs = false;
...
{
    return is4wayParamBufferValid(id) &&
        paramBuffer[PARAM_HEADER_SIG] == ESC_SIG_AM32;
}

static bool isBlheliSParamBufferValid(uint8_t id)
{
    return is4wayParamBufferValid(id) &&
        paramBuffer[PARAM_HEADER_SIG] == ESC_SIG_BLHELI_S;
}
...
bool escCommitParameters(void)
{
    if (is4wayEscSelected()) {
        ...
        } else if (paramBuffer[PARAM_HEADER_SIG] == ESC_SIG_BLHELI_S) {
            blheliSParamWritten[escID] = false;

            if (!isBlheliSParamBufferValid(escID)) {
                return false;
            }
        } else {
            return false;
