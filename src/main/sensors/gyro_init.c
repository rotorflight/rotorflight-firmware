...
static void dynLpfFilterInit()
{
    ...
    if (gyroConfig()->dterm_lpf1_dyn_min_hz > 0) {
        ...
        case FILTER_PT1:
            ...
            break;
        case FILTER_PT2:
            gyro.dynLpfDtermFilter = DYN_LPF_PT2;
            break;
        case FILTER_PT3:
            gyro.dynLpfDtermFilter = DYN_LPF_PT3;
            break;
        case FILTER_BIQUAD:
...
void gyroInitFilters(void)
{
    ...
    uint16_t dterm_lpf1_init_hz = gyroConfig()->dterm_lpf1_static_hz;

#ifdef USE_DYN_LPF
    if (gyroConfig()->gyro_lpf1_dyn_min_hz > 0) {
        gyro_lpf1_init_hz = gyroConfig()->gyro_lpf1_dyn_min_hz;
    }
    if (gyroConfig()->dterm_lpf1_dyn_min_hz) {
        dterm_lpf1_init_hz = gyroConfig()->dterm_lpf1_dyn_min_hz;
    }
#endif
...
void gyroSetLooptime(uint8_t pidDenom, uint8_t filterDenom)
{
    ...
    if (gyro.sampleRateHz) {
        ...
        gyro.filterLooptime = pidDenom * 1000000 / gyro.sampleRateHz;
        gyro.targetLooptime = filterDenom * 1000000 / gyro.sampleRateHz;
        gyro.filterRateHz = gyro.sampleRateHz / filterDenom;
        gyro.targetRateHz = gyro.sampleRateHz / pidDenom;
    } else {
