...
enum {
    ...
    ESC_SENSOR_PROTO_XDFLY,
    ESC_SENSOR_PROTO_FBUS,
    ESC_SENSOR_PROTO_SRXL2,
...
typedef struct {
    ...
    int8_t      consumption_correction;         // Correction for consumption over/under reporting in +-%, 0% = no correction
} escSensorConfig_t;
