...
int usbd_ep_start_read(uint8_t busid, const uint8_t ep, uint8_t *data,
  ...
  /* Arm the endpoint. Do NOT touch the data-toggle bits here: the hardware
   * auto-toggle (UEP_R_TOG_AUTO) alternates DATA0/DATA1 per bulk packet.
   * Forcing DATA0 on every re-arm corrupts toggle sync, drops every second
   * OUT packet and leaves the endpoint NAKed (matches betaflight). */
