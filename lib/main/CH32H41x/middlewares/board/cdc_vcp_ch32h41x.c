...
volatile uint8_t usb_rx_activity = 0;
volatile uint8_t usb_tx_activity = 0;
volatile uint8_t usb_dtr_active = 0;
volatile uint8_t usb_line_coding_set = 0;
volatile uint8_t usb_out_ep_armed = 0;
volatile uint8_t usb_config_event_seen = 0;
...
void usbd_cdc_acm_set_dtr(uint8_t busid, uint8_t intf, bool dtr) {
  (void)busid;
  (void)intf;
  usb_dtr_active = dtr ? 1 : 0;
  if (ctrlLineStateCb) {
    ctrlLineStateCb(ctrlLineStateCbContext, dtr ? 1 : 0);
  }
}
...
void usbd_cdc_acm_set_line_coding(uint8_t busid, uint8_t intf,
  ...
  if (baudRateCb) {
    baudRateCb(baudRateCbContext, line_coding->dwDTERate);
  }
}
