...
The value of 0 is no correction, 1000 is +1.000% correction, -2000 is -2.000%, etc.
...
A special value 0 indicates that the filter task is running at the PID loop speed.
Other values are relative to the gyro speed.

The filter task can't be slower than the PID task. Usually there are 2 or 3 valid values
for this parameter, typically 1..4.
...
Switch between the YAW/WAY D-gain selection criteria on
  (0)  Yaw PID Error
  (1)  Yaw Gyro signal
  (2)  Yaw D-term

Available only in mode 9.
