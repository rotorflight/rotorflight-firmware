...
The governor now has precompensation for the yaw, for main motor driven tails.

There is a new parameter `gov_yaw_ff_weight` for the purpose, working the same as
`gov_cyclic_ff_weight` and `gov_collective_ff_weight`.

For motorised tails, this parameter must be set to zero.
