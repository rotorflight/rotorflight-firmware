...
`vbat_full_cell_voltage` - Cell voltage at which the battery is deemed to be full, in 0.01V units, i.e. 410 = 4.1V
...
Within each profile the thresholds must be ordered `min` <= `warning` <= `full` <= `max`; a profile that is not ordered is reset to the defaults.
...
set vbat_full_cell_voltage = 410,420,410,410,410,410
